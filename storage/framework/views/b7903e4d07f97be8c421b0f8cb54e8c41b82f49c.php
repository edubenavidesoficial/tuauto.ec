 <header> 
            <div class="h2_header-area header-sticky">
                <div class="container">
                    <div class="row align-items-center">
                        <div class="col-xl-3 col-sm-7 col-6">
                            <div class="h2_header-left">
                                <div class="h2_header-logo">
                                    <a href="index.html"><img src="assets/img/logo/logo.png" alt=""></a>
                                </div>
                                <div class="h2_header-category d-none d-sm-block">
                                    <a href="#"><i class="fa-solid fa-grid"></i><span>Redes</span></a>
                                    <ul class="h2_header-category-submenu">
                                        <li><a href="#">Face</a></li>
                                        
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div class="col-xl-6 d-none d-xl-block">
                            <div class="h2_header-middle">
                                <nav class="h2_main-menu mobile-menu" id="mobile-menu">
                                    <ul>
                                        <li >
                                            <a href="/">INICIO</a>
                                          
                                        </li>
                                        <li class="menu-has-child">
                                           <a href="">ESTUDIOS EN ARGENTINA </a>
                                            <ul class="submenu">
                                                <li><a href="">UNIVERSIDADES ARGENTINA</a></li>
                                                <li><a href="">SERVICIOS</a></li>
                                                <li><a href="">REQUISITOS ARGENTINA</a></li>
                                            </ul>
                                        </li>
                                        
                                        <li class="menu-has-child">
                                            <a href="">SOMOS</a>
                                            <ul class="submenu">
                                                <li><a href="">QUIÉNES SOMOS</a></li>
                                                <li><a href="">EXPERIENCIAS</a></li>
                                            </ul>
                                        </li>
                                        <li><a href="">CONTÁCTANOS</a></li>
                                    </ul>
                                </nav>
                            </div>
                        </div>
                        <div class="col-xl-3 col-sm-5 col-6">
                            <div class="h2_header-right">
                                <div class="h2_header-btn d-none d-sm-block">
                                    <a href="#" class="header-btn theme-btn theme-btn-medium">Sign Up</a>
                                </div>
                                <div class="header-menu-bar d-xl-none ml-10">
                                    <span class="header-menu-bar-icon side-toggle">
                                        <i class="fa-light fa-bars"></i>
                                    </span>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </header><?php /**PATH /home/ingealimitec2/public_html/argentina.ingealimite.com/resources/views/web/layout/menu.blade.php ENDPATH**/ ?>